...
def tokenize_and_numericalize_example(example, tokenizer):
    ...
    return {"ids": tokens["input_ids"], "start_label": start_label, "label_vector": label_vector, 'attention_mask': tokens["attention_mask"]}
...
def get_collate_fn(pad_index):
    ...
    def collate_fn(batch):
        ...
        batch = {"ids": batch_ids, "start_label": batch_start_label, "label_vector": batch_label_vector, "attention_masks": batch_attention_masks}
...
def get_data(batch_size):
    ...
    # Tokenize the data
    data = data.map(tokenize_and_numericalize_example, fn_kwargs={"tokenizer": tokenizer})
    
    pad_index = tokenizer.pad_token_id
    
    # Convert arrays to torch Tensor
    data = data.with_format(type="torch", columns=["ids", "start_label", "label_vector", "attention_mask"])
    
    # Create Dataloaders
    data = get_data_loader(data, batch_size, pad_index, shuffle=True)

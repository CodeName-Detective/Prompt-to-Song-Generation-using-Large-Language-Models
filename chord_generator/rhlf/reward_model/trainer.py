...
class Single_Core_Trainer:
    def __init__(self: 'Single_Core_Trainer'):
        self.loss_criteria = torch.nn.MSELoss()
    ...
    def _one_epoch_train(self: 'Single_Core_Trainer', model: torch.nn.Module, data_loader_train: torch.utils.data.DataLoader, 
        ...
        for batch in tqdm(data_loader_train):
            ...
            chord_vector = batch['chord_vector'].to(device=DEVICE)
            ...
            # Forward pass.
            y_pred = model(inputs, chord_vector, attention_mask)
            loss = self.loss_criteria(y_pred, labels)
        ...
        for epoch in tqdm(range(epochs)):
            ...
            with torch.inference_mode():
                for batch in tqdm(data_loader_test, desc="Evaluation:"):
                    ...
                    loss_batch = self.loss_criteria(y_pred, labels)
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter(log_dir='tensorboard_logs/'+'_freeze_'+str(freeze_transformer)+'_lr_'+str(learning_rate).replace('.','p')+'_mpt_'+str(mixed_precision_training))
    
    log_file = 'freeze_'+str(freeze_transformer)+'_lr_'+str(learning_rate).replace('.','p')+'_mpt_'+str(mixed_precision_training)+'.log'
    logger = CustomLogger(os.path.join('logs', log_file))
    
    checkpoint_save_dir = os.path.join('models', 'cd_'+'_freeze_'+str(freeze_transformer)+'_lr_'+str(learning_rate).replace('.','p')+'_mpt_'+str(mixed_precision_training))
    train_data_loader, val_data_loader = get_data(batch_size=batch_size)
    
    model = load_model(freeze=freeze_transformer)
